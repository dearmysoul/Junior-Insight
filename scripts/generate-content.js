...
const LESSON_SCHEMA = {
    ...
    properties: {
        ...
        argument: {
            type: 'object', additionalProperties: false,
            description: '지문을 근거로 입장이 갈릴 수 있는 쟁점 — "나의 주장" 미션용',
            properties: {
                claim: { type: 'string', description: '정답이 하나로 정해지지 않는 한 줄 쟁점 질문 (예: "게임 확률 공개, 꼭 필요할까?")' },
                options: {
                    type: 'array', items: { type: 'string' },
                    description: '아이가 고를 입장 2~3개 (예: ["필요하다","필요없다","잘 모르겠다"]). 마지막은 "잘 모르겠다" 류 유보 선택지.',
                },
            },
...
6. argument는 지문을 근거로 찬반·입장이 갈릴 수 있는 쟁점 질문 하나(claim)와,
   아이가 고를 입장 2~3개(options)를 만들어라. 정답이 하나로 정해지지 않고
   근거로 다툴 수 있는 질문이어야 한다(단순 사실 확인 질문 금지).`;
