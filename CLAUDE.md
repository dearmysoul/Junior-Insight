...
| 4 | 매일 `subject_of_day` 존재 또는 첫 기사 `type==="lesson"` (평일=교과, 주말=일반상식 예술·금융) | 있음 | 지문 대신 뉴스 폴백됨(키/팩트체크 확인 필요) |
...
- **하나라도 실패** → 사용자에게 알림. 무엇이/왜 틀렸는지 + 권장 조치를 한눈에:
  ...
  - 점검 #3·#4·#5 실패 → "날씨 배너 또는 지문(평일 교과/주말 일반상식) 누락·뉴스 폴백. ANTHROPIC_API_KEY(Actions Secret)와 최근 워크플로 로그를 확인하세요."
