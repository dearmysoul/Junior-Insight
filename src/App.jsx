...
/** 순수 '날씨' 기사 판별 — 선정 단계에서 날씨 카드를 1개로 제한하기 위한 플래그
 *  (기후·에너지 등 실질 환경 기사와 구분) */
...
function detectCategory(title) {
    const t = title.toLowerCase();
    // Tech: IT·AI·플랫폼·서비스 + 게임/e스포츠 (아이 관심 소재)
    ...
    // Environment: 기후·에너지 + 날씨
    if (/기후|환경|탄소|온난화|재활용|에너지|원전|신재생|풍력|태양광|탄소중립|해수면|오염|생태|날씨|기상|기온|강수|강우|강설|호우|대설|태풍|장마|폭염|한파|황사|미세먼지|폭우|홍수|가뭄|소나기|안개|천둥|번개|눈 예보|비 예보|낮 최고|밤 최저|아침 기온|나들이|주말 날씨|오늘 날씨|내일 날씨|이번 주 날씨/.test(t)) return 'Environment';
    // Economy: 경제·금융·시장
...
/** 하루치 기사 선정 — 날씨 최대 1개 / 아이 관심(Tech) 최소 1개 / World 최소 1개 /
 *  동일 카테고리 최대 2개 / 총 6개. (fetch-news.js의 selectDaily와 동일 규칙) */
...
/* ──────────────────────────────────────────────
   HANJA TOOLTIP
   ────────────────────────────────────────────── */
// controlled: 열림 상태를 상위(HanjaPassage)가 소유 → 지문 전체에서 한 번에 하나만 열림
function HanjaTooltip({ term, id, openId, setOpenId, children }) {
    const open = openId === id;
    return (
        <span className="relative inline">
            <button
                type="button"
                // 다른 한자어를 누르면 openId가 그 쪽으로 바뀌어 기존 버블은 자동으로 닫힘.
                // stopPropagation: 바깥 클릭 닫기 리스너가 이 클릭까지 삼켜 즉시 닫지 않도록.
...
function HanjaText({ text, terms, keyPrefix, openId, setOpenId }) {
    ...
    return (
        <>
            {parts.map((part, i) => {
                ...
                return match
                    ? <HanjaTooltip key={i} id={`${keyPrefix}-${i}`} term={match} openId={openId} setOpenId={setOpenId}>{part}</HanjaTooltip>
...
/** 지문 전체(여러 단락)를 감싸 한자어 툴팁의 열림 상태를 하나만 유지.
 *  다른 한자어 클릭 시 기존 버블이 닫히고, 지문 밖 클릭 시에도 닫힘. */
function HanjaPassage({ summaryKor, terms }) {
    ...
    return (
        <div className="text-[16px] text-foreground leading-[1.9] tracking-tight space-y-3">
            {summaryKor.split('\n').filter((p) => p.trim()).map((para, pi) => (
                <p key={pi}>
                    <HanjaText text={para.trim()} terms={terms} keyPrefix={`p${pi}`} openId={openId} setOpenId={setOpenId} />
                </p>
            ))}
        </div>
    );
                    ...
                    <div className="bg-card border border-border rounded-xl p-5">
                        ...
                        {news.summary_kor ? (
                            <div className="mb-4">
                                ...
                                <HanjaPassage summaryKor={news.summary_kor} terms={news.hanjaTerms} />
                            </div>
                        ) : (
